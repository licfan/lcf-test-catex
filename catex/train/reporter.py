...
_accumulation_tag = {"losses", "losses_real"}

def to_reported_value(v: Num, weight: Num = None, scalar = False) -> "ReportedValue":
...
def aggregate(values: Sequence["ReportedValue"]) -> Num:
    ...
    elif isinstance(values[0], Scalar):
        retval = values[-1].value
...
class SubReporter:
    ...
    def __init__(self, key: str, epoch: int, total_count: int, batch_num: int):
        assert check_argument_types()
        self.key = key
        self.epoch = epoch
    ...
    def register(
        ...
        for key2, v in stats.items():
            ...
            if v is None:
                v = np.nan
            r = to_reported_value(v, weight, scalar)
            ...
            if key2 not in self.stats:
                ...
                # stat A: [0.4, 0.3, 0.5]
                # stat B: [nan, nan, 0.2]
                nan = to_reported_value(np.nan, None if weight is None else 0, scalar)
    ...
    def log_message(self, start: int = None, end: int = None) -> str:
        ...
        message = f"{self.key}: epoch:{self.epoch}" f" {start + 1}-{end} / {self.batch_num} batch: "

        for idx, (key2, stats_list) in enumerate(self.stats.items()):
            assert len(stats_list) == self.count, (len(stats_list), self.count)
            # values: List[ReportValue]
            if key2 in _accumulation_tag:
                values = stats_list
            else:
...
class Reporter:
    ...
    def __init__(self, epoch: int = 0, batch_num = 0):
        assert check_argument_types()
        if epoch < 0:
    ...
    def start_epoch(self, key: str, epoch: int = None) -> SubReporter:
        ...
        sub_reporter = SubReporter(key, self.epoch, total_count, self.batch_num)
